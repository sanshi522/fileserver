        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.page.content.length; i++) {
                    ...
                    $("#class").children("tbody").append('<tr data_key="id" data_val="' + data.page.content[i].id + '" up="0">\n' +
                        '<td class="serial">' + (i + 1) + '</td>\n' +
                        '<td type="text" notnull="1" state="1" data_key="name" data_val="' + data.page.content[i].name + '">' + data.page.content[i].name + '</td>\n' +
                        '<td type="notext" notnull="1" state="1" data_key="gradeId" data_val="' + data.page.content[i].gradeId + '">' + gradeName + '</td>\n' +
...
function tableadd() {
    var index;
    if ($("#class").children("tbody").children("tr:last").find("td").length == 0)
        index = 1;
    else
        index = ($("#class").children("tbody").children("tr:last").children("td:first").html() - 0) + 1;
    $("#class").children("tbody").append('<tr data_key="id" data_val="" up="0">\n' +
        '<td class="serial">' + index + '</td>\n' +
        '<td type="text" notnull="1" state="1" data_key="name" data_val=""></td>\n' +
        '<td type="notext" notnull="1" state="1" data_key="gradeId" data_val="' + gradeid + '">' + gradeName + '</td>\n' +
        '<td style="color: red;font-size: 20px;"><i class="my-icon lsm-sidebar-icon icon-shanchu del"></i></td>' +
        '</tr>');
    tablebind();
}
