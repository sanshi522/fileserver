...
//        Example<FileSample> example = Example.of(screenShareFile.getFileSample(), matcher);
    ...
    @Override
    public List<Integer> ScreenALL2(ScreenShareFile screenShareFile, HttpServletRequest request) {
        HttpSession session = request.getSession();
...
//            pageable = PageRequest.of(screenShareFile.getPageIndex(), screenShareFile.getPageNumber());
          ...
          FileIdList = shareFileRepository.findFileIdByIdIn(ShareFileIdList);
    ...
    @Override
    public List<Integer> ScreenMyALL2(ScreenShareFile screenShareFile, HttpServletRequest request) {
        HttpSession session = request.getSession();
        List<Integer> FileIdList=new ArrayList<>();
        if (session != null && session.getAttribute("user") != null) {
            SessionUser sessionUser = new SessionUser();
           ...
           FileIdList = shareFileRepository.findFileIdByIdInAndOwnerIdentAndownerId(ShareFileIdList, shareIdent, shareId);//
//            if (screenShareFile.getLikeName().isEmpty())
//                json.put("page", fileSampleRepository.findALLByIdIn(FileIdList, pageable));
//            else
//                json.put("page", fileSampleRepository.findALLByIdInAndNameLike(FileIdList, "%" + screenShareFile.getLikeName() + "%", pageable));
    ...
    @Override
    public Map choiceShareFile(ScreenShareFile screenShareFile, List<Integer> list) {
        Map json = new HashMap();
        Sort sort;
        Pageable pageable;
        if (!screenShareFile.getSort().isEmpty()) {
        ...
        if (screenShareFile.getLikeName().isEmpty()) {
            json.put("page", fileSampleRepository.findALLByIdIn(list, pageable));
        }
            else {
            json.put("page", fileSampleRepository.findALLByIdInAndNameLike(list, "%" + screenShareFile.getLikeName() + "%", pageable));
        }
